...
- 2026-09-22: Re-scanned the fresh official AI, agent, robotics, VLA/manipulation, and infrastructure sources after the prior run; no newly verified item cleared the publication threshold, so the Research Feed was left unchanged.
...
- 2026-09-23: Added Microsoft Research's physical-AI inference offloading study and toolchain after primary-source verification. Across mobile-manipulation workloads, the study shows that compute placement itself can change task success, latency, and battery life; smaller onboard GPUs caused up to 383% mapping/planning slowdown and VLA accuracy degradation in the evaluated setup. This displaced Isaac ROS 5.0 in the same-day top-three and creates a useful systems contrast with Nota's fully on-device NPU optimization: offload versus edge is a workload/network/control-loop trade-off, not a universal architecture choice.
